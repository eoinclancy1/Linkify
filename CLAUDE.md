...
- **Company mention detection**: `detectCompanyMention(text, companyUrl, companyName?)` checks for the URL slug (e.g. "airopshq") AND the company name (e.g. "airops"). Both are needed because they differ — the URL slug alone misses employee posts that mention the company by name. The `companyName` is threaded from `AppConfig` through `scrapePostsForProfiles()` → `mapPostToDatabase()` → `detectCompanyMention()`. The orchestrator's `updateCompanyMentions()` also runs a repair pass to retroactively flag employee posts whose `mentionsCompany` was missed.
...
- **Ones to Watch page** (`/ones-to-watch`): Shows external prospects (no Employee record) who mentioned AirOps in the last 14 days. Fetches from `/api/mentions?range=14&external=true`, deduplicates by author name (keeps highest engagement), classifies by `externalAuthorHeadline` into 3 sections: Marketing Leaders, SEO/Organic Leaders, Other Senior Leaders. Authors below director level are excluded. Uses `ProspectCard` component (`src/components/ones-to-watch/ProspectCard.tsx`) with `<img>` tags (not Next.js Image) for external avatar URLs. Search bar filters by name/headline. Empty sections are hidden.
